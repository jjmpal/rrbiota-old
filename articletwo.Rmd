...
lacto.full <- full_join(lacto.meta, lacto.abund, by = "id") %>%
...
lacto.mean <- lacto.full %>%
    group_by(fake_salt) %>%
    summarize(estimate = t.test(lactobacillus.bacteria)$"estimate",
              qi_low = t.test(lactobacillus.bacteria)$"conf.int"[1],
              qi_high = t.test(lactobacillus.bacteria)$"conf.int"[2],
              group = "mean")

lacto.odds <- lacto.full %>%
    group_by(lacto_groups) %>%
    do(oddsmodel = glm(HYPERTENSION ~ lactobacillus.bacteria, family=binomial(link="logit"), data = .)) %>%
    do(data.frame(
        fake_salt = .$lacto_groups,
        term = names(coef(.$oddsmodel)),
        exp(cbind(coef(.$oddsmodel), confint(.$oddsmodel))))) %>%
    dplyr::mutate(group = "odds") %>%
    dplyr::rename(estimate = V1, qi_low = X2.5.., qi_high = X97.5..) %>%
    filter(!grepl("Intercept", term)) %>%
    select(-term)
```


```{r forest plot lacto, eval = FALSE}

myforestplot <- function(data) {
    ggplot(data = data,
           aes(x = fake_salt, y = estimate, ymin = qi_low, ymax = qi_high)) +
        facet_wrap(~group,strip.position="top",nrow=1,scales = "free_y") +
        xlab('Group') +
        ylab("Risk Ratio (95% Confidence Interval)")+
        geom_pointrange()+
        geom_hline(aes(fill=fake_salt), yintercept=1, linetype=2)+
        geom_errorbar(aes(ymin=qi_low, ymax=qi_high),width=0.2,cex=1)+ 
...
gsalt <- grid.arrange(myforestplot(lacto.mean),  myforestplot(lacto.odds), ncol=2)
ggsave(file = "cache/salt-forest.png", plot=gsalt, height=6, width=9)
```
