...
```{r deseq2, eval = FALSE}
pseq.genus.core.deseq <- pseq.genus %>% core(detection = 0.0001, prevalence = 0.01)

dds.data <- phyloseq_to_deseq2(pseq.genus.core.deseq,
                               as.formula(sprintf("~ %s + %s", "MAP", paste0(var.CL, collapse = "+"))))

#dds = DESeq(dds.data, test="Wald", fitType="parametric")
#resultsNames(dds)
#res = results(dds)


ds2 <- estimateSizeFactors(dds.data, type="poscounts")
dds <- DESeq(ds2)

res <- results(dds)
df <- as.data.frame(res)
df$taxon <- rownames(df)
  #df <- df %>% arrange(pvalue) # TODO

  df$model.str <- modelstr
  df$covariate <- variables[nv]
...
```{r bptreat numbers}
pseq.species %>% meta %>% pull(BP_TERAT) %>% summary
```

Hypertension prevalence

```{r test htn prevalence}
dset.alpha %>%
    dplyr::filter(BL_AGE < 60 + 12, BL_AGE > 60 - 12) %>%
    summarize(htn = sum(ifelse(HYPERTENSION == 1, 1, 0)), n = n(), proportion = htn/n) %>%
    kable
```
