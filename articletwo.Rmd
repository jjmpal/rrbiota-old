...
dimensions (`r dim(meta(pseq.genus))`) and there ntaxa is `r
ntaxa(pseq.genus)`.
...
if (!file.exists("rds/maaslin.rds")) {
    ...
    pseq.species %>% meta %>% pull(DIAB) %>% summary
    pseq.genus.core %>% meta %>% pull(DIAB) %>% as.factor %>% summary
...
pcoa.df %>% pull(HYPERTENSION) %>% summary
...
data(dune)
data(dune.env)
adonis(dune ~ Management, data=dune.env, permutations=99)$aov.tab
adonis(dune ~ Management, data=dune.env, permutations=99)$coefficients
```

```{r age sex bmi}
lm(MAP ~ BL_AGE + SEX + BMI, data = dset.alpha) %>% summary
```
