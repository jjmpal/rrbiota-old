...
g.deseq <- results(dds, contrast = c("HYPERTENSION", 0, 1)) %>%
    ...
    dplyr::mutate(qval = p.adjust(pvalue, method="BH")) %>%
    dplyr::filter(qval < 0.05) %>%
    dplyr::mutate(group = dplyr::ntile(row_number(), 3)) %>%
    ggplot(aes(x=Feature, y=log2FoldChange, color=qval, group = group)) +
...
ggsave(file = "cache/deseq.png", plot=g.deseq, height=10, width=15)
```
