...
```{r data}
pseq.genus <- import_filter_data("data/phfinrisk_centrifuge_genus_all_drop50k_2018-11-30.RDs")
pseq.species <- import_filter_data("data/phfinrisk_centrifuge_species_all_2018-11-30.RDs")
```
...
```{r check adonis, eval = FALSE}
ad.min <- adonis(bray.dist.m.species ~ DIASM + BL_AGE + SEX,
             data = meta(pseq.species),
             permutations = 100)

ad.max <- adonis(bray.dist.m.species ~ DIASM + BL_AGE + SEX + BMI + CURR_SMOKE + Q57X + DIAB +
                     BL_USE_RX_C03 + BL_USE_RX_C07 + BL_USE_RX_C08 + BL_USE_RX_C09,
             data = meta(pseq.species),
             permutations = 1000)
...
if (!file.exists("rds/maaslin.rds")) {
    ...
    maaslin$genus <- maaslinwrapper(pseq = pseq.genus.core,
                                    looped = c("MAP", "SYSTM", "DIASM", "PULSEPRESSURE",
                                               "HYPERTENSION"),
                                    forced = c("BL_AGE", "SEX", "BMI", "CURR_SMOKE", "Q57X",
                                               "DIAB", "BL_USE_RX_C03",
...
merge(diversities$min %>% select(response, R2),
      diversities$max %>% select(response, R2), by = "response") %>%
    mutate(R2.diff = R2.y - R2.x) %>%
    kable
```
...
Results for beta diversity

```{r table beta diversity}
adonis.species$max %>%
    data.table::rbindlist(id = "model_name") %>%
    dplyr::filter(term %in% c("MAP", "SYSTM", "DIASM", "PULSEPRESSURE", "HYPERTENSION")) %>%
    dplyr::select(response, R2, "Pr(>F)") %>%
    mutate(p.value = pub.p(`Pr(>F)`)) %>%
    kable
```
...
abund.genus.bacteria <- colnames(abund.genus)[-1] %>% mygrep(".bacteria$", .) 

pcoacorabund <- function(names, pcoa) {
    lapply(c2l(abund.genus.bacteria), function (name) 
                     lapply(c2l(1,2,3), function(index) {
                             model <- lm(abund.genus[,name] ~ pcoa[,index])
                             summary(model)$r.squared
                         }))
}

pcoacor.genus <- pcoacorabund(abund.genus.bacteria, pcoa.vectors)
...
calculateglm(pcoa.df,
             modelstring = "%s ~ Axis.1 + BL_AGE + SEX + BMI + CURR_SMOKE + Q57X + DIAB +
                     BL_USE_RX_C03 + BL_USE_RX_C07 + BL_USE_RX_C08 + BL_USE_RX_C09",
             filterstr = "Axis") %>% 
...
Testing associations to blood pressure variables on PCoA axis 2

```{r pcoa axis analysis axis 2}
calculateglm(pcoa.df,
             modelstring = "%s ~ Axis.2 + BL_AGE + SEX + BMI + CURR_SMOKE + Q57X + DIAB +
                     BL_USE_RX_C03 + BL_USE_RX_C07 + BL_USE_RX_C08 + BL_USE_RX_C09",
             filterstr = "Axis") %>% 
    select(response, term, estimate, std.error, p.value) %>%
    mutate_if(is.numeric, round, 3) %>%
...
Testing associations to blood pressure variables on PCoA axis 3

```{r pcoa axis analysis axis 3}
calculateglm(pcoa.df,
             modelstring = "%s ~ Axis.3 + BL_AGE + SEX + BMI + CURR_SMOKE + Q57X + DIAB +
                     BL_USE_RX_C03 + BL_USE_RX_C07 + BL_USE_RX_C08 + BL_USE_RX_C09",
             filterstr = "Axis") %>%
    select(response, term, estimate, std.error, p.value) %>%
    mutate_if(is.numeric, round, 3) %>%
...
theta <- 130
phi <- 10
pcoa3d.plot <- ggplot(data=pcoa.df, aes(x=Axis.1, y = Axis.2, z=Axis.3, color = HYPERTENSION)) +
  axes_3D(theta=theta, phi=phi) +
  stat_3D(theta=theta, phi=phi, size = 0.01, alpha=0.4) +
  labs_3D(theta=theta, phi=phi, 
          hjust=c(1,0,0), vjust=c(1.5,1,-.2),
          labs=c(sprintf("PCoA Axis 1 (%.1f%%)", 100*eig.fracs[1]),
                 sprintf("PCoA Axis 2 (%.1f%%)", 100*eig.fracs[2]),
                 sprintf("PCoA Axis 3 (%.1f%%)", 100*eig.fracs[3]))) #+
#  theme_void()

ggsave(file = "cache/pcoa-species-3d.png", plot=pcoa3d.plot, height=4, width=7, units = "in", dpi = 300)
...
Catabacter, Robinsoniella

```{r compare maaslin to sun, eval = FALSE}
maas.results.df.all <- maaslin$genus %>%
    merge(names.dset %>% select(Covariate, Category, Name), by.x ="Variable", by.y = "Covariate") %>%
    dplyr::mutate(Qval = p.adjust(P.value, method="BH"),
                  Feature = gsub("_Bacteria", "", Feature),
                  beta = Coefficient)

maas.results.df.all %>% pull(Feature) %>% sort %>% unique %>% mygrep("Robin", .)

genus.taxa <- pseq.genus.core %>% abundances %>% rownames

genus.taxa %>% mygrep("Catabacter", .)

maaslin.test <- maaslinwrapper(pseq = pseq.genus.core,
                                    looped = c("MAP", "SYSTM", "DIASM", "PULSEPRESSURE",
                                               "HYPERTENSION"),
                                    forced = c("BL_AGE", "SEX", "BMI", "CURR_SMOKE", "Q57X",
                                               "DIAB", "BL_USE_RX_C03",
                                               "BL_USE_RX_C07", "BL_USE_RX_C08", "BL_USE_RX_C09"),
                               taxa = paste(c("Faecalicatena.Bacteria",
                                        "Catabacter.Bacteria",
                                        "Robinsoniella.Bacteria"), collapse = ", "))
