...
pseq.genus.core <- microbiome::transform(pseq.genus, "compositional") %>%
    core(detection = 0.001, prevalence = 0.01)
```
...
if (!file.exists("rds/dds.rds")) {
    ...
    dds <- DESeq(dds.data, test="Wald", fitType="parametric", parallel = TRUE, BPPARAM=MulticoreParam(16))
...
```{r deseq results}
dset.deseq.signf <- results(dds, contrast = c("HYPERTENSION", 0, 1)) %>%
    as.data.frame %>%
...
dset.deseq.boxplot <- pseq.genus %>%
    ...
    prune_taxa(pull(dset.deseq.signf, Feature), .) %>%
    psmelt %>%
    mutate(Abundance = sqrt(Abundance))
```

```{r deseq results numerical}
dset.deseq.signf %>% kable
```

```{r deseq plot}
g.deseq <- dset.deseq.signf %>%
    ggplot(aes(x=gsub("g_", "", Feature), y=log2FoldChange, color=qval)) +
    geom_point(size=1) +
    ...
    xlab("Feature") +
    theme_classic()
ggsave(file = "cache/deseq.png", plot=g.deseq, height=8, width=5)
```
...
g.boxplot <- dset.deseq.boxplot %>%
    filter(OTU %in% pull(dset.deseq.signf  %>% dplyr::filter(abs(log2FoldChange) > 1), Feature)) %>%
    group_by(OTU) %>%
    ...
    geom_point(data = function(x) dplyr::filter_(x, ~ outlier),
               position = position_jitterdodge(jitter.width = 0.2), size = 0.01, alpha = 0.4) +
    scale_y_continuous(limits = c(0, 0.1)) +
    scale_x_discrete(labels=c("0" = "Normotensive", "1" = "Hypertensive")) +
    ylab("Sqrt For Relative Abundances") +
    theme_classic() +
...
```{r maaslin insignif, include = FALSE, eval = FALSE}
 maaslin %>%
    dplyr::mutate(Qval = p.adjust(P.value, method="BH"),
                  Feature = gsub("[gs]_", "", Feature),
                  beta = Coefficient) %>%
     arrange(Feature) %>%
     select(Variable, Feature, Qval) %>%
...
ggsave(file = "cache/maaslin.png", plot = maaslinplot, height = 3, width = 4, unit = "in", dpi = 300)
```
...
scatter.plot <- function(df,
                         ...
                         rr,
                         abund,
                         covariates = c("BL_AGE", "SEX", "BMI", "CURR_SMOKE", "Q57X", "PREVAL_DIAB",
...
```{r maaslin means, eval = FALSE}
pseq.genus.core.table <- pseq.genus.core %>%
...
rbind(table_bp_abund(pseq.genus.core.table, "map_groups"),
             table_bp_abund(pseq.genus.core.table, "systm_groups")) %>%
