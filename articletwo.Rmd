...
if (!file.exists("rds/dds.rds")) {
    ...
    dds.data <- phyloseq_to_deseq2(pseq.genus.core.deseq, ~HYPERTENSION)
...
```{r deseq results table}
results(dds, contrast = c("HYPERTENSION", 0, 1)) %>%
    as.data.frame %>%
...
```{r deseq plot}
g.deseq <- results(dds, contrast = c("HYPERTENSION", 0, 1)) %>%
    as.data.frame %>%
    tibble::rownames_to_column("Feature") %>%
    dplyr::mutate(qval = p.adjust(pvalue, method="BH")) %>%
    dplyr::filter(qval < 0.05) %>%
    ggplot(aes(x=Feature, y=log2FoldChange, color=qval)) +
    geom_point(size=1) +
...
ggsave(file = "cache/deseq.png", plot=g.deseq, height=10, width=5)
```
