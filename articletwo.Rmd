...
scatter.meta <- meta(pseq.genus) %>%
    tibble::rownames_to_column("id") %>%
      mutate(MAP = oMAP,
           ...
           SYSTM = oSYSTM,
           DIASM = oDIASM) %>%
    gather("keym", "valuem", c(SYSTM, DIASM, MAP, PULSEPRESSURE))

scatter.abund <- abundances(pseq.genus, transform = "compositional") %>%
    t %>%
    as.data.frame %>%
    tibble::rownames_to_column("id") %>%
...
scatterplot <- ggplot(scatter.full, aes(y = valuem, x = value)) +
    facet_wrap(~key + keym, ncol=4, scales = "fixed") +
