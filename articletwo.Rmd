...
sne_names <- colnames(sne.df) %>% mygrep(word = "g_")


sne.t <- sne.df %>% select(Sample, HYPERTENSION, sne_names[1:20]) %>%
    mutate(HYPERTENSION = as.numeric(HYPERTENSION)) %>%
    gather(variable, value, -Sample) %>% 
    spread(Sample, value) %>%
    column_to_rownames("variable")

png("sample.png", 800, 800)
tsne(sne.t[-1,], labels = as.factor(sne.t[1,]),
     dotsize = 1)
dev.off()
...
sne.t <- sne.df %>% select(Sample, oMAP, sne_names) %>%
    gather(variable, value, -Sample) %>% 
    spread(Sample, value) %>%
    column_to_rownames("variable")

tsne.g <- tsne(sne.t[rownames(sne.t) != "oMAP",],
               labels = as.numeric(sne.t[rownames(sne.t) == "oMAP",]),
               dotsize = 1,
               controlscale = TRUE,
               scale = 2,
               low = "white")
...
tsne.plot <- tsne.g +
    scale_fill_gradientn(limits = c(-3,3),
                         ...
                         breaks=c(0,200), labels=format(c(0,200)))

ggsave(file = "cache/tsne.png", plot=tsne.plot, height=6, width=9)
