...
map_df(res.trim, ~do.call(rbind, .x)) %>%
    dplyr::mutate(Qval = p.adjust(p.value, method="BH")) %>%
    arrange(Qval)

maaslin <- do.call(rbind, lapply(c2l(var.BP), function(BP) res[[BP]]))
...
```{r maaslin}
maaslin <- read.table("maaslin1/maaslin/maaslin.txt", header=TRUE, sep='\t')

maaslin %>% filter(Q.value < 0.05)
```
