...
pcoa.plot <- ggplot(data=pcoa.df, aes(x=Axis.1, y=Axis.2, color = STATETREATMENT, shape = STATETREATMENT)) +
    ...
    ylab(sprintf("PCoA Axis 2 (%.1f%%)", 100*eig.fracs[2])) +
...
Load genus data with dU-NA and combine it in a data frame with clr transformed lactobacillus abundances.
...
Characteristics in salt analysis
...
Association between lactobacillus and salt groups

```{r abund salt lm group}
lm.lacto.saltgroup <- lm(lactobacillus.bacteria ~ salt_groups + BL_AGE + SEX, data = df.genus.salt)
lm.lacto.saltgroup %>% tidy %>% kable
```


Assesment for linear model asuumptions
...
```{r equal variance}
bartlett.test(lactobacillus.bacteria ~ salt_groups, data = df.genus.salt)
```

```{r one way test salt groups}
oneway.test(lactobacillus.bacteria ~ salt_groups, data = df.genus.salt, var.equal = TRUE)
```

Anova between lactobacillus and salt groups
...
Tukey's range test for lactobacillus and salt groups
...
Calculate adjusted means for lactobacillus 
...
adjmean.lacto.saltgroup <- predict(lm.lacto.saltgroup,
    ...
    dplyr::rename(estimate = fit,
                  conf.low = lwr,
                  conf.high = upr) %>%
    rownames::tibble_to_column("term")
```
...
glm.hypertension.lactogroup %>% kable
```

```{r glm htn lacto continuous}
glm.hypertension.lactocontinuous <- glm(HYPERTENSION ~ lactobacillus.bacteria + BL_AGE + SEX,
                  family = binomial(link=logit),
                  data = df.genus.salt) %>%
    broom::tidy(conf.int = TRUE, conf.level = 0.95, exponentiate = TRUE)
glm.hypertension.lactocontinuous %>% kable
```

Daily urine sodium and BP

```{r dUNA RR}
calculateglm(df.genus.salt, modelstring = "%s ~ dUNA + BL_AGE + SEX", filterstr = "dUNA") %>%
...
Assesment for linear model asumptions

```{r table map lactobacillus}
gvlma.table.plot(lm(log(MAP) ~ lactobacillus.bacteria + BL_AGE + SEX, data = df.genus.salt))$table %>% kable
```
