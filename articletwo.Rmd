...
pseq.genus.salt <- pseq.genus %>%
    prune_samples(rownames(subset(meta(.), !is.na(NA.))), .) %>%
    prune_taxa(c("g_Lactobacillus", "g_Lactococcus"), .)

abund.genus.salt <- pseq.genus.salt %>%
    abundances(., transform = "clr") %>%
    t %>%
    as.data.frame %>%
    tibble::rownames_to_column("id") %>%
    select(-g_Lactococcus)
...
bartlett.test(g_Lactobacillus ~ salt_groups, data =
df.genus.salt)$p.value`. 

One-way test assuming normal distributions gives for equal mean p = `r
oneway.test(g_Lactobacillus ~ salt_groups, data =
