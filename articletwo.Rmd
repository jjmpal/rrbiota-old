...
<details>
  <summary>Toggle imported core</summary>
### Officer functions
...
```{r save grob}
alpha.diversity.min <- plot.alpha(diversities$min, colorlimit = 0.5)
alpha.diversity.max <- plot.alpha(diversities$max, colorlimit = 0.5)
...
fig2foot <- sprintf("Principal component analysis for bacterial abundances at species level. Centered log ratio transform is performed for the compositional data. First two axes explain %.1f%% of the variation. Centered log-ratio (CLR) transformation is used for microbiome abundances.", sum(pca.percentage[1:2]))
...
```{r Write figure 4}
fig4head <- "Associations between dietary salt, lactobacillus abundance, and odds for hypertensoin"
fig4foot <- "In panel A we see lactobacillus abundances in different dietary salt groups. The mean value is adjusted using age and sex. In panel B we split participants in four groups by lactobacillus abundances and caclulate odds ratio for hypertension. Age and sex are included in logistic model. A 95 % CI is drawn in both panels."
