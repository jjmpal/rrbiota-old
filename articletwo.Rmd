...
if (!file.exists("rds/maaslin.rds")) {
    ...
    pseq.genus.core.viruses <- taxa(pseq.genus.core.comp) %>% mygrep(word = "virus$", complement = TRUE)
    pseq.genus.core.comp.pruned <- prune_taxa(pseq.genus.core.viruses, pseq.genus.core.comp)
    
    res <- lapply(c2l(var.BP), function(BP)
        fit_asine(pseq.genus.core.comp.pruned, variables = c(BP, var.CL)))

    saveRDS(res, file = "rds/res.rds")
    
##     res.trim <- map_df(res, ~map_df(.x, ~tidy(.x), .id = "taxa"), .id = "model")

##     adonis <- res.trim %>%
##         filter(term %in% var.BP) %>%
##         group_by(model) %>%
...
## adonis %>% pull(taxa) %>% sort

##     adonis %>% dplyr::filter(taxa == "g_Sutterella")
    
## map_df(res.trim$HYPERTENSION, ~
