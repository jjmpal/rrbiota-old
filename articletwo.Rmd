...
writeimage <- function(doc, filename, header, footer, width = 7.5) {
    img <- readPNG("cache/pca.png")
    doc <- doc %>% 
        body_add_fpar(fpar(ftext("Figure . ", prop = text.bold), ftext(header, prop = text.normal)), style = "Image Caption") %>%
...
```{r Characteristics}
tbl1 <- xtableformatter(characteristics(dset$fr02$data.full)) %>%
...
tbl1foot <- "Continuous variables are presented as mean (standard deviation). Categorical variables reported as absolute and relative frequencies. BMI indicates body mass index."
...
writeimage(doc, "cache/alpha-beta.png", "Alpha-beta", "footer")
```
...
writeimage(doc, "cache/pca.png", "Alpha-beta", "footer", width = 7)
```
...
pca.loading %>% pull(PC3) %>% head(2700) %>% sum

ggplot(data=pca.pc3, aes(x=reorder(term, -PC3), y=PC3)) +
