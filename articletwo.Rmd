...
lacto.mean <- lacto.full %>%
    ...
    summarize(estimate = t.test(lactobacillus.bacteria)$"estimate",
              conf.low = t.test(lactobacillus.bacteria)$"conf.int"[1],
              conf.high = t.test(lactobacillus.bacteria)$"conf.int"[2]) %>%
    dplyr::rename(term = fake_salt)

lacto.odds <- glm(HYPERTENSION ~ lacto_groups, family = binomial(link=logit), data=lacto.full) %>%
...
myforestplot <- function(data, xlab = "", ylab = "", ylimits = c(0,10), interceptone = FALSE) {
    ggplot(data = data,
           aes(x = term, y = estimate, ymin = conf.low, ymax = conf.high)) +
        geom_point() +
        { if (interceptone) geom_hline(aes(fill=fake_salt), yintercept=1, linetype=2) } +
        geom_errorbar(width=0.1,cex=1)+ 
