...
maas.results.df.signf <- maaslin$genus %>%
    ...
    dplyr::mutate(Qval = p.adjust(P.value, method="BH"),
                  ...
                  beta = Coefficient) %>%
    dplyr::filter(Qval < 0.05, Variable != "HYPERTENSION") %>%
    arrange(Feature)

my.plot.order <- function(x) {
    y <- seq(length(x))
    y[x == "Mean arterial pressure"] = 4
    y[x == "Systolic BP"] = 3
    y[x == "Diastolic BP"] = 2
    y[x == "Pulse pressure"] = 1
    y
}
...
maaslinplot <- ggplot(maas.tables.df, aes(x = Feature, y = reorder(Name, my.plot.order(Name)), fill = beta)) +
    ...
    ylab("") +
    theme_classic(10) +  
    theme(axis.text.x = element_text(angle = 90, hjust = 1.0, vjust=0.5, size=7),
...
fig3foot <- "Only backteria with significant associations are shown. Blood pressure variables are normalized and abundances are arcsin-square root transformed. Dark gray color represents unsignificant associations."
