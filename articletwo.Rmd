...
```{r lactobacillus scatterplot,  results = 'hide'}
pseq.lacto  <- subset_taxa(pseq.species, Genus=="Lactobacillus")
maaslin.force.vars.lacto <- c("BL_AGE", "SEX", "BMI", "CURR_SMOKE", "Q57X", "DIAB", "BL_USE_RX_C03", "BL_USE_RX_C07", "BL_USE_RX_C08", "BL_USE_RX_C09")

prepare.maaslin(pseq.lacto,
                 tsvfile = "maaslin_lacto_merged_phfinrisk_genus.tsv",
                conffile = "maaslin_lacto_config.config",
                includedvars = c("MAP", "SYSTM", "DIASM", "PULSEPRESSURE", "HYPERTENSION", maaslin.force.vars.lacto),
...
Maaslin("maaslin_lacto_merged_phfinrisk_genus.tsv",
        ...
        dSignificanceLevel = 1.0, 
        fAllvAll = TRUE,
        strForcedPredictors = maaslin.force.vars.lacto)
```
...
betadiversity$min$model_1 %>% select(term, R2)
betadiversity$max$model_1 %>% select(term, R2)


```

Max values for alpha diversity `r diversities$max %>% pull(estimate)
								  %>% abs %>% max %>% round(., 2)` and
								  `r diversities$mmin %>%
								  pull(estimate) %>% abs %>% max %>%
								  round(., 2)`.
...
```{r Reading maaslin lacto output}
