...
axis_labeller <- function(variable, value){
    axis.labels <- list("Axis.2" = sprintf("A. PCoA Axis 2 (%.1f%%)", 100*eig.fracs[2]),
                        "Axis.3" = sprintf("B. PCoA Axis 3 (%.1f%%)", 100*eig.fracs[3]))
    return(axis.labels[value])
}

pcoa.plot <- ggplot(data=pcoa.df, aes(x=Axis.1, y=Axis, color = HYPERTENSION)) +
    facet_wrap(~key, labeller = axis_labeller) +
    geom_point(size=0.1, alpha=0.4) +
    xlab(sprintf("PCoA Axis 1 (%.1f%%)", 100*eig.fracs[1])) +
    ylab(sprintf("Subsequent PCoA Axes", 100*eig.fracs[3])) +
...
ggsave(file = "cache/pcoa-species.png", plot=pcoa.plot, height=4, width=6, units = "in", dpi = 300)
```
...
```{r top generas, eval = FALSE, include = FLASE}
...
maas.tables.df %>% pull(beta)%>% summary
...
```{r scatter plot at genus level for BP, fig.width=12, fig.height=10}
...
gsalt <- grid.arrange(
    myforestplot(adjmean.lacto.saltgroup,
                 ...
                 subtitle = "A.",
                 my_y_scale = scale_y_continuous(limits = c(4.1,4.7), breaks = seq(4, 5, 0.1))),
