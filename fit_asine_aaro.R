...
fit_asine_aaro_my_na_filter <- function(phf_f, variables) {
...
#'
#' @export
fit_asine_aaro_remove_unnec_vars <- function(variables, phf_f, fixed.variable) {
...
fit_asine_aaro_fit_adonis <- function (phf.rel, variables, save.intermediate=FALSE, intermediate.name="phyla",
                        dist.matrix=NULL, ...) {
  # dist.matrix if supplied must be dist object and contain labels

  # filter out rows with na's
  phf.rel <- fit_asine_aaro_my_na_filter(phf.rel, variables)

  variables <- as.vector(fit_asine_aaro_remove_unnec_vars(variables, phf.rel, last(variables)))
...
fit_asine <- function(phf_f, variables, intermediate.name="_asin1_", extra.printouts=FALSE,
                      save.intermediate=FALSE, intermediate.list.file="data_private/processed_asine.txt",
                      return.only.filename=FALSE) {

  phf_f <- fit_asine_aaro_my_na_filter(phf_f, variables)
  cat("\nafter na filter, fitting asin(sqrt(OTU))")

  variables <- as.vector(fit_asine_aaro_remove_unnec_vars(variables, phf_f, last(variables)))
...
fit_asine_aaro_get_fit <- function (fits.all, vi, read.from.files) {
  if (read.from.files) {
...
collect_lm_stats <- function (our_vars, otu_names, fits.all, read.from.files=FALSE) {
  ...
  for (vi in 1:length(our_vars)) {
    varname <- our_vars[[vi]]
    fits <- fit_asine_aaro_get_fit(fits.all, vi, read.from.files)
