...
```{r maaslin runs, results = 'hide', eval  = TRUE} 
ret <- mclapply(var.CL.comb, function(x)
    maaslinwrapper(pseq = pseq.genus.core,
                   looped = var.BP,
                   forced = x,
                   taxa = taxa(pseq.genus.core),
                   tempstr = sprintf("%%s/maaslinruns/combinations/%%s_%s", paste0(x, collapse = "_"))),
...
```{r read combinations, eval = TRUE}
ret <- lapply(var.CL.comb, function(x) {
    file <- sprintf("/homes/jpalmu/maaslinruns/maaslinruns/1560334017_%s/maaslin/maaslin.txt",
                    paste0(x, collapse = "_"))
...
```{r adding third, eval = FALSE}
ret3rd <- mclapply(var.CL.comb3rd, function(x)
    maaslinwrapper(pseq = pseq.genus.core,
                   looped = var.BP,
                   forced = x,
                   taxa = taxa(pseq.genus.core),
                   tempstr = sprintf("%%s/maaslinruns/maaslinruns/%%s_%s", paste0(x, collapse = "_"))),
    mc.cores = length(var.CL.comb3rd))

```

```{r read combinations 3rd, eval = TRUE}
ret3rd <- lapply(var.CL.comb3rd, function(x) {
    file <- sprintf("/homes/jpalmu/maaslinruns/maaslinruns/1560338417_%s/maaslin/maaslin.txt",
                    paste0(x, collapse = "_"))
    read.table(file, header=TRUE, sep='\t') %>% as.data.frame
    })
```
