...
```{r function for glm calculation, include = FALSE}
...
```{r function for adnoiscalculation, include = FALSE}
calculateadonis <- function(dset,
                            ...
                            npermutations = 99) {
    glmlist <- mclapply(responses, function(response) {
        fo <- as.formula(sprintf(modelstring, response, covariates))
        ad <- adonis(fo, data = dset, permutations = npermutations)
        ad$aov.tab %>% as.data.frame
    }, mc.cores = lenfth(responses)) 
}
```
...
adonis(bray.dist.m.species ~ MAP + BL_AGE + SEX + BMI + CURR_SMOKE + Q57X + DIAB + BL_USE_RX_C03 + BL_USE_RX_C07 + BL_USE_RX_C08 + BL_USE_RX_C09,
       data = meta(pseq.species),
       permutations = 2)$aov.tab %>%
                            as.data.frame %>%
                            kable
