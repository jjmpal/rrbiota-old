...
filter.phenotype.data <- function(pseq,
 ...
 #       dplyr::filter(DNA_OK == 1) %>%
        stats::na.omit() %>%
        dplyr::select(-MEN, -DNA_OK, -BP_TREAT) %>%
...
Phenotypical data between genus and species all.equal `r isTRUE(all.equal(dset.genus, dset.species))`. Dimension for data
...
dset.species %>%
    mutate(ANYDRUG = ifelse(BL_USE_RX_C09  == 1 | BL_USE_RX_C03 == 1 |
                            BL_USE_RX_C07 == 1  | BL_USE_RX_C08 == 1, 1, 0),
           EXERCICE = ifelse(Q57X == 1, 0, 1)) %>%
    group_by(SEX, CURR_SMOKE,  HYPERTENSION, ANYDRUG, EXERCICE) %>%
    select(SEX, CURR_SMOKE, HYPERTENSION, EXERCICE) %>%
    summarise(n = n()) %>%
    arrange(desc(n)) %>%
    kable

dset.species %>% pull(Q57X) %>% summary
```
