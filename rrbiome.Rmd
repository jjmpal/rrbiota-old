...
calculateglm <- function(dset,
    ...
    data.table::rbindlist(test, id = "model_name") %>%
        as.data.frame %>%
        dplyr::mutate(conf.low = estimate - qnorm(1- 0.05/2) * std.error,
                      conf.high = estimate + qnorm(1- 0.05/2) * std.error)
}

calculateglm(as.data.frame(dset.alpha), modelstring = "%s ~ BL_AGE + SEX + BMI + CURR_SMOKE + Q57X + DIAB + BL_USE_RX_C03 + BL_USE_RX_C07 + BL_USE_RX_C08 + BL_USE_RX_C09  + diversities_shannon") %>%
    select(response, estimate, conf.low, conf.high, p.value) %>%
    kable

#lm(MAP ~ BL_AGE + SEX + BMI + CURR_SMOKE + Q57X + DIAB + BL_USE_RX_C03 + BL_USE_RX_C07 + BL_USE_RX_C08 + BL_USE_RX_C09  + diversities_shannon, data = dset.alpha) %>% summary()
...
```{r beta hclust, fig.width = 5, fig.height = 4, dpi = 300, eval = FALSE}
...
```{r PCA plots continuous, fig.width = 10, fig.height = 10, dpi = 300,  eval = FALSE}
