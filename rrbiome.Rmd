...
get.merged.dataset <- function(pseq) {
    ...
    t(rbind(sample.ids, pheno, abud.rel))
}
...
prepare.maaslin <- function (pseq,
    ...
    write.table(get.merged.dataset(pseq), file = tsvfile, sep='\t', quote=FALSE, row.names=FALSE, col.names=TRUE)
...
maas.results.df <- merge(maas.results.df, names.dset[,c('Covariate','Category', 'Name')], by=c('Covariate'))
...
ggplot(df.merged %>% gather(Covariate, relabundance, sysm.signf.features), aes(x = asin(sqrt(relabundance)), y = SYSTM)) +
