...
meta.merge.alphadiversity <- function(pseq, index = "shannon") {
    ...
    base::merge(meta(pseq), alphadiversity, by=0, all=TRUE) %>%
        dplyr::rename(Sample_ID = Row.names) %>%
            dplyr::mutate(BL_AGE_GROUP = group_age(dset.alpha$BL_AGE))
}
```
...
```{r alpha-MAP scatter plots, fig.width=10, fig.height=4}
ggplot(dset.alpha %>% mutate(SEX = factor(ifelse(SEX == 0, "Male", "Female"))),
       aes(x=diversities_shannon, y=MAP)) +
...
```{r alpha diversity scatterplot, fig.width=10, fig.height=4, eval = FALSE}
ggplot((dset.alpha %>%
...
lm(MAP ~ BL_AGE + SEX + diversities_shannon, data=dset.alpha) %>%
    summary(.) %>% coef %>% kable(., digits=4)
lm(MAP ~ BL_AGE + SEX + BMI + CURR_SMOKE + Q57X + diversities_shannon, data=dset.alpha) %>%
    summary(.) %>% coef %>% kable(., digits=4)
