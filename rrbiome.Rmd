...
calculateadonis <- function(dset,
    ...
    mclapply(responses, function(response) {
        fo <- paste(response, covariates, sep = " + ", collapse = " + ")
        ad <- adonis(as.formula(paste(matrix, fo, sep = "~")), data = dset, permutations = npermutations)
        ad$aov.tab %>%
            as.data.frame %>%
...
#pseq.core <- microbiome::core(pseq.genus, detection = 0.001, pervalence = 0.01)
...
maaslin.lacto.force.vars <- c("BL_AGE", "SEX", "BMI", "CURR_SMOKE", "DIAB")

prepare.maaslin(pseq.lacto,
                 tsvfile = "maaslin_lacto_merged_phfinrisk_genus.tsv",
                conffile = "maaslin_lacto_config.config",
                includedvars = c("MAP", "SYSTM", "DIASM", "PULSEPRESSURE", "HYPERTENSION", maaslin.lacto.force.vars),
                readpclrows = "Lactobacillus_acetotolerans.Bacteria-")
...
Maaslin("maaslin_lacto_merged_phfinrisk_genus.tsv",
        ...
        dSignificanceLevel = 1.0, 
        fAllvAll = TRUE,
        strForcedPredictors = maaslin.lacto.force.vars)
```
