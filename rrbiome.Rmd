...
if (!exists('phfinrisk_metadatadesc')) {
  # this is a hand-compiled data.frame that contains metadata information for variables utilized in our analyses
  data("phfinrisk_metadatadesc", package="finriskmetagcommon")
}
names.dset <- filter(phfinrisk_metadatadesc, Ignored.Covariate.in.Cross.Sectional.Analysis.Aaro20181115==0)
...
```{r alpha diversity scatterplot, fig.width=8, fig.height=4}
...
Matriisin teko _bc_dissimilarity_matrix.R ja käyttö comp_adonis_covariates.R
...
```{r beta heatmap, eval = FALSE}


ggplot(data = dist.matrix, aes(x = metrics, y = teams)) +
