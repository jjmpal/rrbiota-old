...
```{r pca percentage}
(pca.prcomp$sdev / sum(pca.prcomp$sdev)) %>% head %>% kable
```

Labels for PCA plot containing the 

```{r pca labels}
pca.percentage <- round(pca.prcomp$sdev / sum(pca.prcomp$sdev) * 100, 2)
pca.labels <- paste0(colnames(pca.prcomp$x), " (", paste0(as.character(pca.percentage), "%", ")"))
```
