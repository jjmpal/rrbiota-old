...
ggsave(file="cache/plotlist.png", g, height=8*length(plotlist)/1.5, width=10*2)
```

<img src="cache/plotlist.png"/>

# Alpha scatter

```{r draw alpha distribution scatter plot}
...
<img src="cache/ggpairs-alphas.png"/>
...
```{r beta diversity pvalue}
betadf <- data.frame()
for (key in names(rds)) {
    moddf.full <- Reduce(function(...) merge(..., all=T), ret[[key]][["betadiversity"]]$full) %>%
        dplyr::filter(response == term) %>%
        mutate(model = key, covariates = "full") %>%
        select(term, model, covariates, `Pr(>F)`)
    moddf.min <- Reduce(function(...) merge(..., all=T), ret[[key]][["betadiversity"]]$min) %>%
        dplyr::filter(response == term) %>%
        mutate(model = key, covariates = "min") %>%
        select(term, model, covariates, `Pr(>F)`)
...
betadf %>% arrange(term) %>% spread(model, `Pr(>F)`) %>% kable
```
