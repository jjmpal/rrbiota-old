...
if (!file.exists("rds/comparepseqs_bray_matrix.rds")) {
    bray.dist.matrix <- mclapply(pseq, function(x) calculate.beta.matrix(x), mc.cores = 4)
    saveRDS(bray.dist.matrix, file = "rds/comparepseqs_bray_matrix.rds")
} else {
    bray.dist.matrix  <- readRDS("rds/comparepseqs_bray_matrix.rds")
}
```
