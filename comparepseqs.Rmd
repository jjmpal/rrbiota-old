...
diversities$min
    ...
    alpha.diversity.full <- plot.alpha(diversities$full, colorlimit = 0.6)
    ...
    gs4 <- list(alpha.diversity.min$yaxis,
                ...
                text_grob(paste("A.", key), size = 18, just = "left"),
                ...
                text_grob(paste("B.", key), size = 18, just = "left"))
...
g <- arrangeGrob(plotlist[[1]], plotlist[[2]], ncol=1)
ggsave(file="cache/plotlist.pdf", g, height=6*length(plotlist), width=10)
```

```{r debug, eval = FALSE}
ret[["centrifuge"]]$betadiversity$full$model_1
```
