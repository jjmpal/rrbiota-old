...
pseq.species.askbp <- subset_samples(pseq.species,
                                       PREVAL_DIAB == 0 &
                                       PREVAL_CHD == 0 &
                                       PREVAL_CR_ANYCANC == 0) 

bray.dist.m.species.askbp <- bray.dist.m.species[pseq.species.askbp %>% meta %>% rownames,
                                                   pseq.species.askbp %>% meta %>% rownames]

alphadiversity.askbp <- calculate.alphadiversity(pseq.species.askbp,
                                                   list("max" = var.CL.opt,
                                                        "min" = var.CL.min))

betadiversity.askbp <- calculate.betadiversity(pseq = pseq.species.askbp,
                                                matrix = bray.dist.m.species.askbp,
